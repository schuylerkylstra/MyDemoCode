...
public:
	sphere(triple p, float r, surface c)
	{
		center = p;
		radius = r;
		material = c;
	...
	{
		material = surface(RGB, aCo, dCo, sCo, sPow, 0.f);
	}
	...
	float intersectionTime(ray * r)
	{
		// center, origin (0,0,0), radius, ray r
		return quadEQ(2*((r->direction)*(r->origin) + -1*((r->direction)*center)) , (r->origin)*(r->origin) + center*center - 2*((r->origin)*center) - radius*radius);
	}


	spaceVector getNormal(spaceVector location)
	{
		spaceVector normal = location - center;	
		normal.normalize();
		return normal;
	}
...
private:
	...
	float quadEQ(float b, float c) // a is always 1 because the ray directions are normalized
	{
