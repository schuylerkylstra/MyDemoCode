...
/*
            ...
            int gLeftPixelValue = helper.roundToInt(gLeft), gRightPixValue = helper.roundToInt(gRight);
 */
            uint32_t DstColor;
            uint32_t lclColor;
            float lclXVal;
            ...
            for (int j = top; j< bottom; j++) {
                ...
                for (int i = left; i <right; i++) {
                    ...
                    char* gAddress = (char*) Btmp.fPixels + gRowsDown +  i;
                    DstColor = *((GPixel*) gAddress);
                    
                    invMtx.vectorMult(&lclXVal, &lclYVal);
                    xVal = (int) floorf(lclXVal);
                    yVal = (int) floorf(lclYVal);
                    lclColor = *((GPixel*) (char*) lclBmp.fPixels + yVal*lclBmp.fRowBytes + xVal);
