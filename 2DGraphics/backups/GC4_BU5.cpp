...
public:
    ...
    void drawBitmapIdent(const GBitmap&, float x, float y, const GPaint&, const int a);
    ...
    if(intAlpha != 0){
        if (!mapMtx.isIdentity()){
            ...
            if((left != right) && (top != bottom)){ //if intersection surface area is zero, do nothing
                ...
                float lclYVal = top + .5f;
                float lclXStart = left + .5f;
                invMtx.vectorMult(&lclXStart, &lclYVal);
                float lclXVal = lclXStart;
                float lclXInc = 1.f;
                float lclYInc = 1.f;
                invMtx.vectorMult(&lclXInc, &lclYInc);
                for (int j = top; j< bottom; j++) {
                    for (int i = 0; i <intervalWidth; i++) {
                        DstColor = *((GPixel*) gRow + i);
                        lclColor = *((GPixel*) ((char*) lclBmp.fPixels) + (int)floorf(lclYVal)*lclBmp.fRowBytes + (int)floorf(lclXVal));
                ...
                for(int j= top; j< bottom; j++){
                    ...
                    for(int i = left; i<right; i++){
                        yVal = lclYVal;
                        lclXVal = i + .5f;
                        invMtx.vectorMult(&lclXVal, &yVal);
                        gLocal = ((char*) Btmp.fPixels) + j*Btmp.fRowBytes + i;
                        lclLocal =((char*) lclBmp.fPixels) + ((int)yVal)*lclBmp.fRowBytes + ((int)lclXVal);
                        DstColor = *((GPixel*) gLocal);
                        lclColor = 0xFF00FF00;//*((GPixel*) lclLocal);
                        *((GPixel*) gLocal) = lclColor;
                        /*if (lclColor != 0) { //if there is something to blend
                            if(intAlpha == 1){
                                *((GPixel*) gLocal) = helper.blendColor(DstColor, lclColor);
                            }
                            else if( DstColor == 0 ) { //if destination alpha == 0
                                
                                *((GPixel*) gLocal) = helper.packARGB(lclColor, intAlpha);
                            }
                            else{
                                *((GPixel*) gLocal) = helper.blendColorWithAlpha(DstColor, lclColor, intAlpha);
                            }
                        }*/
...
void GContext4::drawBitmapIdent(const GBitmap& lclBmp, float x, float y, const GPaint& paint, int intAlpha){
    float gTop = y, gLeft = x, gBottom = y+lclBmp.fHeight, gRight = x+lclBmp.fWidth;
    int left = helper.roundToInt(gLeft);
    int right = helper.roundToInt(gRight);
    int top = helper.roundToInt(gTop);
