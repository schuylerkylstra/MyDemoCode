...
public:
    ...
    static inline int roundToInt(float input){
        return floorf(input + .5f);
    }
    ...
    void inline findIntersection(int* left, int* right, int* top, int* bottom, int height, int width){
        *top = myMax<int>(*top, 0);
        *bottom = myMin<int>(*bottom,height);
        *left = myMax<int>(*left,0);
        *right = myMin<int>(*right,width);
    }
};
